import { getTargetActors, i18n } from './betterrolls_kryx_rpg.js'

Hooks.on('renderChatMessage', (message, html, data) => {
  if (!game.settings.get('betterrolls_kryx_rpg',
    'chatDamageButtonsEnabled')) { return }
  ...
  function addButtons (dmgElement) {
    ...
    let fullDamageButton = $(
      `<button data-modifier="1"><i class="fas fa-user-minus" title="${i18n(
        'brkr.chat.damageButtons.fullDamage.hint')}"></i></button>`)
    let halfDamageButton = $(
      `<button data-modifier="0.5"><i class="fas fa-user-shield" title="${i18n(
        'brkr.chat.damageButtons.halfDamage.hint')}"></i></button>`)
    let doubleDamageButton = $(
      `<button data-modifier="2"><i class="fas fa-user-injured" title="${i18n(
        'brkr.chat.damageButtons.doubleDamage.hint')}"></i></button>`)
    let fullHealingButton = $(
      `<button data-modifier="-1"><i class="fas fa-user-plus" title="${i18n(
        'brkr.chat.damageButtons.healing.hint')}"></i></button>`)
  ...
  html.find('.dmgBtn-container-br button').click(async ev => {
    ...
    setTimeout(() => {
      if (canvas.hud.token._displayState && canvas.hud.token._displayState !==
        0) {
