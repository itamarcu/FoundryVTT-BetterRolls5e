...
export class BetterRollsHooks {
	...
	*/
	static registerActorSheet( // SHEET CREATORS SHOULD NO LONGER USE THIS!
		...
		Hooks.on(sheetString, (app, html, data) => {
			game.settings.get("betterrolls_kryx_rpg", "rollButtonsEnabled") ? addItemSheetButtons(app.object, html, data, triggeringElement, buttonContainer) : null;
			game.settings.get("betterrolls_kryx_rpg", "diceEnabled") ? changeRollsToDual(app.object, html, data, params) : null;
		});
	}
	...
	static registerItemSheet(sheetName) { // SHEET CREATORS SHOULD NO LONGER USE THIS!
		let sheetString = "render" + sheetName;
		Hooks.on(sheetString, (app, html, data) => {
			game.settings.get("betterrolls_kryx_rpg", "diceEnabled") ? addBetterRollsContent(app, html, data) : null;
		});
	}
	...
	*/
	static addItemContent(actor, html, 
		...
		(game.settings.get("betterrolls_kryx_rpg", "rollButtonsEnabled") && triggeringElement && buttonContainer) ? addItemSheetButtons(actor, html, null, triggeringElement, buttonContainer) : null;
		(game.settings.get("betterrolls_kryx_rpg", "diceEnabled") && itemButton) ? changeRollsToDual(actor, html, null, {itemButton: itemButton}) : null;
	}
}
...
Hooks.on("renderedMagicItems", (actor, html, data) => {
	if (window.MagicItems && game.settings.get("betterrolls_kryx_rpg", "diceEnabled")) {
